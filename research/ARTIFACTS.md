...
8. **Matryoshka NLA public artifacts exist** (no paper): the term was coined
   in the length-penalty post's comments (nostalgebraist/Karvonen), and syvb
   ships `nla-qwen2.5-7b-L20-{av,ar}-matryoshka-sonnet46` — our exact
   model+layer — plus `nla-ordered-features-{av-sft,ar-sft,rl}`. The
   referenced PDF remains unpublished/unlocated → still ask the user; treat
   as confidential if provided.
9. **Frozen-artifact reproducibility is not currently executable**: the
...
8. "Matryoshka NLAs" PDF (per session instructions, "in the docs directory")
   is absent, and no public paper by that name exists — likely the
   importance-ordered-explanations line behind the syvb nanoNLA collection.
   Need the file from the user.
...
## 2026-07-19 — Amendment 1 §11 revisions applied (header restored 2026-07-19; commit c0db220)
