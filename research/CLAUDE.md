...
| Building Better Activation Oracles | https://arxiv.org/abs/2606.02609 · code https://github.com/japhba/activation_oracles · HF https://huggingface.co/collections/ceselder/building-better-activation-oracles | Bauer et al. 2026 — on-policy AO training, improved injection formula, multi-layer feeds, and AObench (eval suite; its "Activation Sensitivity" task — same tokens, different upstream context — is direct precedent for our crossed pairs). The "every run that did NLA-style injection performed significantly worse" (Qwen3-8B) claim is explicitly **not a formal ablation** (their words), and nanoNLA's maintainer has since walked back confidence that additive beats replacement — treat additive-vs-replacement as unsettled. |
...
| EasyNLA (asherps) | https://github.com/asherps/EasyNLA | Distributed nanoNLA fork, Qwen3-8B L24. **Different injection default from official** (verified in `nla/utils/vllm_steer.py` 2026-07-19): additive norm-matched at the layer-1 output, `h'_p = h_p + ‖h_p‖·v/‖v‖` (Karvonen formula), via a patched vllm-lens SteeringVector. The repo also carries the official embedding-replacement hook (`nla/injection.py`) but the trained/rollout path is additive. |
| nanoNLA (ceselder) | https://github.com/ceselder/nanoNLA | Minimal NLA reimplementation (Qwen3-8B L24, LoRA, vllm-lens; Apache-2.0). Same additive layer-1 injection deviation; README states the maintainer is **no longer confident** additive is significantly better than replacement. |
